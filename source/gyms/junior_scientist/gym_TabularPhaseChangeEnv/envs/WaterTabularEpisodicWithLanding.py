...
class WaterTabularEpisodicWithLanding(gym.Env):
    def __init__(self):
        self.action_space = gym.spaces.Discrete(4) #Move left, stay, move right, land (declair that you have reach the goal)
        self.RoomTemp = 21 #degC
        self.gen_critPoints()
        self.rewardAtGoal = 10
    ...
    def step(self, action):
        ...
        if action == 0 and self.stateID > 0:
            self.stateID -= 1
        elif action == 2 and self.stateID < len(self.Energies)-1:
            self.stateID += 1
        elif action == 3:
            if self.atGoal():
                self.done = True
